...
class _RxDraftPageState extends State<RxDraftPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Draft Prescription', style: TextStyle(color: Colors.white)), centerTitle: true, backgroundColor: Colors.blue),
      body: SafeArea(
            ...
            child: Card(
              ...
              child: ExpansionTile(
                ...
                title: Row(
                  ...
                  children: [
                    Expanded(child: SizedBox(height: screenHeight / 8, width: screenWidth / 6, child: Image(fit: BoxFit.cover, image: FileImage(File(finalImage))))),
                    const SizedBox(width: 30),
                    Expanded(flex: 2, child: Column(children: [Text("${user[index].docName} (${user[index].docId}) ", overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), Text(user[index].areaName, style: const TextStyle(fontSize: 13))])),
                  ],
                ),
                children: [
                  Row(
                    ...
                    children: [
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                content: const Text("Are you sure you want to Delete The RX?"),
                                actions: [
                                  TextButton(
                                    onPressed: () {
                      ...
                      TextButton.icon(
                        onPressed: () {
                          ...
                          filteredMedicin = [];
                          addedRxMedicinList.where((item) => item.uiqueKey == dcrKey).forEach((item) {
                            debugPrint('gsp: ${item.uiqueKey}');
                            final temp = MedicineListModel(uiqueKey: item.uiqueKey, strength: item.strength, brand: item.brand, company: item.company, formation: item.formation, name: item.name, generic: item.generic, itemId: item.itemId, quantity: item.quantity);

                  ...
                  itemBuilder: (context, index) {
                    ...
                    return InkWell(
                      onTap: () async {
                        ...
                        await Navigator.of(context).push(MaterialPageRoute(builder: (_) => NoticePage(notice: noticeList[index])));
                        setState(() {
                          noticeIds = seenNoticeCount.get('notice_Id', defaultValue: []);
                          noticeList = noticeBox.values.toList();
                          noticeCount = (noticeList.length - noticeSeenCount.length).toString();
                            ...
                            child: Row(
                              ...
                              children: [
                                Container(
                                  height: 60,
                                  width: 80,
                                  decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.bottomCenter, end: Alignment.centerRight, colors: [Color.fromARGB(255, 138, 201, 149), Color.fromARGB(255, 119, 199, 133), Color.fromARGB(255, 62, 201, 91)]), borderRadius: BorderRadius.circular(15)),
                                  child: Padding(
                                    padding: const EdgeInsets.fromLTRB(5, 8, 0, 2),
                                    child: Row(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Expanded(child: FittedBox(child: Text(lastPart!, style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w500, color: Colors.white)))),
                                        Expanded(child: FittedBox(child: Padding(padding: const EdgeInsets.all(6), child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [Text(secPart!, style: const TextStyle(fontWeight: FontWeight.w500, color: Colors.white)), const SizedBox(height: 5), Text(firstPart!, style: const TextStyle(fontWeight: FontWeight.w500, color: Colors.white))])))),
                                      ],
                                    ),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(prefixTime!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.black)),
                                      Stack(children: [const Divider(), noticeIds.contains(noticeList[index].notice_id) ? const SizedBox.shrink() : const Positioned(right: 4, top: 0, bottom: 0, child: Align(alignment: Alignment.centerRight, child: Icon(Icons.notifications_active, color: Colors.green, size: 24)))]),
                                      Text(noticeList[index].notice_title, textAlign: TextAlign.justify, style: const TextStyle(fontSize: 16)),

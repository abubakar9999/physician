...
Future ResetPass(sync_url, oldPass, newPass, conPass, context) async {
  ...
  final response = await http.post(
    ...
    body: jsonEncode(<String, String>{"cid": cid, "user_id": user_id, "user_pass": user_pass, "device_id": device_id, "old_pass": oldPass, "new_pass": newPass, "con_pass": conPass}),
  );
  ...
  if (statusCode == 200) {
    ...
    if (data['status'] == 'Success') {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${data['ret_str'].toString()}')));
      // Navigator.push(
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('InCompleted Reset Password')));
  }
  return "Null";
}
...
Future<List> getAreaPage(areaPageUrl, String cid, String userId, String userPassword) async {
  print(
    ...
    //'http://192.168.100.198:8000/hamdard_api/api_area/area_list?cid=$cid&user_id=$userId&user_pass=$userPassword',
  );
  List arePageList = [];

  try {
    final http.Response res = await http.get(
      Uri.parse('$areaPageUrl?cid=$cid&user_id=$userId&user_pass=$userPassword'),
      //'http://192.168.0.153:8000/hamdard_api/api_area/area_list?cid=$cid&user_id=$userId&user_pass=$userPassword'),
      // 'http://192.168.100.198:8000/hamdard_api/api_area/area_list?cid=$cid&user_id=$userId&user_pass=$userPassword',
      //),
      headers: <String, String>{'Content-Type': 'appliscation/json; charset=UTF-8'},
    );
...
Future<bool> getAreaBaseClient(BuildContext context, String syncUrl, cid, userId, userPassword, areaId, areaName) async {
  print('client List : $syncUrl/api_client/client_list?cid=$cid&user_id=$userId&user_pass=$userPassword&area_id=$areaId');
  List clientList = [];
  try {
    final http.Response res = await http.get(Uri.parse('$syncUrl/api_client/client_list?cid=$cid&user_id=$userId&user_pass=$userPassword&area_id=$areaId'), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'});
    ...
    if (clientStatus == 'Success') {
      ...
      await Navigator.push(context, MaterialPageRoute(builder: (_) => CustomerListScreen(terrorId: areaId, data: clientList, terrorName: areaName)));
      return false;
    } else {
      return false;
    }
...
Future getTarAch(BuildContext context, String userSalesCollAchUrl, cid, userId, userPassword, deviceId) async {
  debugPrint('$userSalesCollAchUrl?cid=$cid&user_id=$userId&user_pass=$userPassword&device_id=$deviceId');
  try {
    final http.Response response = await http.get(Uri.parse('$userSalesCollAchUrl?cid=$cid&user_id=$userId&user_pass=$userPassword&device_id=$deviceId'), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'});
    ...
    if (status == 'Success') {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => TargetAchievement(tarAchievementList: tarAchievementList)));
    } else {
...
Future getOrderHistory() async {
  try {
    final http.Response response = await http.get(Uri.parse('http://w305.yeapps.com/acme_api/api_ord_history/ord_history'), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
Future getInvoiceHistory() async {
  try {
    final http.Response response = await http.get(Uri.parse('http://w305.yeapps.com/acme_api/api_inv_history/inv_history'), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
Future getOutStandingDetails() async {
  try {
    final http.Response response = await http.get(Uri.parse('http://w305.yeapps.com/acme_api/api_os_details/os_details'), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
Future timeTracker(String location, cid, userid, userpass, deviceid, timer_track_url) async {
  try {
    var _url = "$timer_track_url?cid=$cid&user_id=$userid&user_pass=$userpass&device_id=$deviceid&locations=$location";
...
///******************************** Attendance  ******************************************///
///******************************** ********************************************************///
Future attendanceAPI(BuildContext context, String submitType, reportAttendance, userName, mtrReading, double? lat, double? long, String address) async {
  print(1);
  ...
  String latitude = (minLatitude <= lat! && lat <= maxLatitude) ? lat.toString() : '';
  String longitude = (minLongitude <= long! && long <= maxLongitude) ? long.toString() : '';
  String attendanceAddress = ((minLatitude <= lat && lat <= maxLatitude) && (minLongitude <= long && long <= maxLongitude)) ? address : '';
  String url = "${sync_url}api_attendance_submit_meter/submit_data?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&submit_type=$submitType&meter_reading=$mtrReading&exp_data=$finalExpenseStr&app_version=$appVersion&latitude=$latitude&longitude=$longitude&address=$attendanceAddress";
  // String url = "http://192.168.100.198:8000/physician_api/api_attendance_submit_meteKr/submit_data?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&submit_type=$submitType&meter_reading=$mtrReading&exp_data=$finalExpenseStr&app_version=$appVersion&latitude=$latitude&longitude=$longitude&address=$attendanceAddress";
  ...
  debugPrint("attendance api:::${sync_url}api_attendance_submit_meter/submit_data?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&submit_type=$submitType&meter_reading=$mtrReading&exp_data=$finalExpenseStr&app_version=$appVersion&latitude=$latitude&longitude=$longitude&address=$attendanceAddress");
  ...
  if (reportAttendance == true && isclick) {
    ...
    if (data["status"] == "Success") {
      ...
      if (submitType.toString() == 'START') {
        ...
        await _secureStorage.write(key: 'background_service', value: "${background_service.toString()}");
      ...
      await databox.put('meter_reading_last', meter_reading_last.isNotEmpty ? meter_reading_last : '0');
      ...
      //     :
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => MyHomePage(userName: userName, user_id: user_id, userPassword: user_pass)));
      ...
      if (data['ret_str'].toString().contains('http')) {
        ...
        update_app_url = data['ret_str'].toString().substring(index).trim() ?? '';
      }
    ...
    Fluttertoast.showToast(msg: 'End Time has been Submitted for Today', toastLength: Toast.LENGTH_LONG, gravity: ToastGravity.SNACKBAR, backgroundColor: Colors.red, textColor: Colors.white, fontSize: 16.0);
  }
}
...
expenseEntry() async {
  ...
  try {
    final http.Response response = await http.get(Uri.parse('$expenseType?cid=$cid&user_id=$user_id&user_pass=$user_pass'), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
SubmitToExpense(expDAteforSubmit, itemString) async {
  try {
    // await getboxData();
    debugPrint("$expenseSubmit?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date=$expDAteforSubmit&exp_data=$itemString");

    final http.Response response = await http.get(Uri.parse("$expenseSubmit?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date=$expDAteforSubmit&exp_data=$itemString"), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
ExpenseReport() async {
  getboxData();
  debugPrint("${report_exp_log}expense_history_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id");

  try {
    final http.Response response = await http.get(
      Uri.parse("${report_exp_log}expense_history_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id"),
...
Future<List<NoticeListModel>> noticeEvent() async {
  ...
  try {
    ...
    final http.Response response = await http.get(
      // Uri.parse(
      // "http://192.168.100.244:8000/hamdard_api/api_notice/notice_list?cid=HAMDARD&user_id=9001&user_pass=1234"),
      Uri.parse("$sync_notice_url?cid=$cid&user_id=$user_id&user_pass=$user_pass"),
      headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'},
    );
    ...
    List<NoticeListModel> noticeList = (noticeDetails['noticeList'] as List).map((item) => NoticeListModel.fromJson(item)).toList();
...
expensapprovel({String? cid, ff_type, user_id, user_pass, device_id, exp_date_from, exp_date_to, exp_url}) async {
  debugPrint("${exp_url}expense_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type");
  ...
  try {
    final response = await http.get(
      Uri.parse("${exp_url}expense_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type"),
      // "http://192.168.0.102:8000/ipi_combined_api/api_expense_approval/expense_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type"),
      headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'},
    );
...
expenseApproveAndReject({String? cid, ff_type, user_id, user_pass, device_id, exp_date_from, exp_date_to, exp_url, status, exp_string, reasonController}) async {
  debugPrint("${exp_url}update_status_emp_exp_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type&emp_id_list=$exp_string&status=$status&reject_reason=$reasonController");
  Response? data;
  var dat;
  try {
    final Response data = await http.get(Uri.parse("${exp_url}update_status_emp_exp_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type&emp_id_list=$exp_string&status=$status&reject_reason=$reasonController"), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'});
...
ExpEmpDetailsList({String? cid, ff_type, user_id, user_pass, device_id, exp_date_from, exp_date_to, exp_url, emp_id}) async {
  debugPrint("${exp_url}emp_expense_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&emp_id=$emp_id");

  try {
    final response = await http.get(Uri.parse("${exp_url}emp_expense_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type&emp_id=$emp_id"), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'});
...
Future expenseEmpApproveAndRejectDetels({String? cid, ff_type, user_id, user_pass, device_id, exp_date_from, exp_date_to, exp_url, emp_id, emp_id_exp_sl_list, status, reasonController}) async {
  debugPrint("${exp_url}update_status_emp_exp_sl_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type&emp_id=$emp_id&emp_id_exp_sl_list=$emp_id_exp_sl_list&status=$status&reject_reason=$reasonController");
  Response? data;
  var dat;
  try {
    final Response data = await http.get(
      Uri.parse("${exp_url}update_status_emp_exp_sl_list?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date_from=$exp_date_from&exp_date_to=$exp_date_to&ff_type=$ff_type&emp_id=$emp_id&emp_id_exp_sl_list=$emp_id_exp_sl_list&status=$status&reject_reason=$reasonController"),
      headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8'},
    );
...
///********************************Expense Summary *******************************///
///******************************** ***************************************************************************///
ExpenseSummary({String? initialDate, String? lastDate, String? ffType, String? ffuserId}) async {
  getboxData();

  try {
    var _url = "${report_exp_log}expense_summary_by_exp_head?cid=$cid&user_id=$user_id&user_pass=$user_pass&date_from=$initialDate&date_to=$lastDate&ff_type=$ffType&ff_user_id=$ffuserId";
    print(_url);
    final http.Response response = await http.get(Uri.parse(_url), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
AttendanceMtrHistory() async {
  ...
  try {
    final http.Response response = await http.get(Uri.parse("$reportAttMtr?cid=$cid&user_id=$user_id&user_pass=$user_pass"), headers: <String, String>{'Content-Type': 'Application/json; charset=UTF-8'});
...
attendanceApprovalAPI(BuildContext context, String late_reason) async {
  ...
  var body = jsonEncode(<String, String>{'cid': cid, 'user_id': user_id, 'user_pass': user_pass, 'note': late_reason});
  ...
  try {
    ...
    if (status == "Success") {
      Fluttertoast.showToast(msg: 'Request submitted successfully', toastLength: Toast.LENGTH_SHORT, gravity: ToastGravity.SNACKBAR, backgroundColor: const Color.fromARGB(255, 121, 192, 153), textColor: Colors.white, fontSize: 16.0);
      Navigator.pop(context, MaterialPageRoute(builder: (_) => AttendanceScreen()));
      return "Success";
    } else {
      Fluttertoast.showToast(msg: "Something went wrong. \n Please try again!", toastLength: Toast.LENGTH_SHORT, gravity: ToastGravity.SNACKBAR, backgroundColor: Colors.red, textColor: Colors.white, fontSize: 16.0);
      return "error";
    }
  } catch (e) {
    debugPrint("Failed!");
...
lateAttendanceReportApi() async {
  ...
  var body = {'cid': cid, 'user_id': user_id, 'user_pass': user_pass};

  try {
    Response response = await http.get(Uri.parse("${late_attendance_url}late_atte_req_report?cid=$cid&user_id=$user_id&user_pass=$user_pass"));
    var bodyData = jsonDecode(response.body);
    ...
    if (status == "Success") {
      ...
      LateAttendanceReportModel dataList = LateAttendanceReportModel.fromJson(bodyData);
      print(dataList.dataList);
      return dataList.dataList;
    } else {
      Fluttertoast.showToast(msg: "Something went wrong", toastLength: Toast.LENGTH_SHORT, gravity: ToastGravity.SNACKBAR, backgroundColor: Colors.red, textColor: Colors.white, fontSize: 16.0);
      return "error";
    }
  } catch (e) {
    debugPrint("Failed!");
...
Future<bool> getAreaBaseDoctor(BuildContext context, String syncUrl, cid, userId, userPassword, areaId, areaName) async {
  print('client List : ${syncUrl}api_sup_dcr/sup_dcr?cid=$cid&user_id=$userId&user_pass=$userPassword&territory=$areaId');
  ...
  try {
    final http.Response res = await http.get(
      Uri.parse('${syncUrl}api_sup_dcr/sup_dcr?cid=$cid&user_id=$userId&user_pass=$userPassword&territory=$areaId'),
    ...
    if (clientStatus == 'Success') {
      ...
      print("AAAAAAAAAAAAAA8******$areaId");
      await Navigator.push(context, MaterialPageRoute(builder: (_) => DcrListPage(visitOfficeDataList: doctorList, branchId: areaId, branchName: areaName)));
      return false;
    } else {
      return false;
    }
...
Future<List<String>> getFFList({String? cid, String? userId, String? userPass, String? ffType}) async {
  var _url = "http://ww11.yeapps.com/ipi_api/api_expense_summary/expense_summary_ff?cid=$cid&user_id=$userId&user_pass=$userPass&ff_type=$ffType";
  print(_url);
...
Future expenseReSubmit({required String expenseData, required String date, required String refSL}) async {
  await getboxData();
  String _url = "${sync_url}api_expense_edit_submit/submit_data_edit_exp?cid=$cid&user_id=$user_id&user_pass=$user_pass&device_id=$device_id&exp_date=$date&ref_sl=$refSL&exp_data=$expenseData";
  print(_url);
